...
//            }
            ...
            // principlaDetailsService의 loadUserByUsername가 실행된 후 정상이면 authentication리턴됨
            // db에있는 username과 password가 일치한다.
            Authentication authentication = authenticationManager.authenticate(authenticationToken);

            //  로그인이 되었다는 뜻
            PrincipalDetails principalDetails = (PrincipalDetails) authentication.getPrincipal();
            System.out.println("로그인완료 :"+principalDetails.getUser().getUsername());  //정상 로그인
            //authentication객체가 세션영역에 저장을 해야하고 리턴해줌
    ...
    // attemptAuthentication인증이 정상적으로 완료되면 함수가 실행됨
    // JWT 만들어서 request사용자에게 JWT리턴
    @Override
